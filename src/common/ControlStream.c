...
static void referenceFrameControlFunc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&invalidateRefFramesThread)) {
        PQUEUED_REFERENCE_FRAME_CONTROL qfit;
        uint32_t invalidateStartFrame = 0;
        uint32_t invalidateEndFrame = 0;
...
int startControlStream(void) {
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        if (client == NULL) {
            Limelog("enet_host_create() returned NULL!\n");
            stopping = true;
            return -1;
        }
        ...
        //
        // PSP NOTE: ENET_SOCKOPT_QOS is skipped on PSP. The PSP network stack returns EAGAIN
        // (error 11) when QoS is enabled, which immediately causes the ENet peer to disconnect
        // before any data is exchanged. PSP-1000 hardware does not support IP DSCP/TOS QoS marking.
        enet_socket_set_option (client->socket, ENET_SOCKOPT_QOS, 1);
#else
        Limelog("PSP: Skipping ENET_SOCKOPT_QOS (not supported on PSP-1000 network stack)\n");
#endif

        {
            // PSP-1000: Retry ENet connection up to 3 times with backoff.
            // The PSP network stack is non-blocking internally and the first
            // serviceEnetHost() may receive a transient DISCONNECT (event 2 / error 11 EAGAIN)
            // before the remote host is ready to accept the ENet handshake.
            int enet_retries;
#ifdef _PSP
            int enet_max_retries = 3;
#else
            int enet_max_retries = 1;
#endif
            for (enet_retries = 0; enet_retries < enet_max_retries; enet_retries++) {
                if (enet_retries > 0) {
                    Limelog("PSP: ENet connect attempt %d/%d (backoff 500ms)...\n", enet_retries + 1, enet_max_retries);
                    PltSleepMs(500);

                    // Rebuild peer for next attempt
                    enet_peer_reset(peer);
                    peer = enet_host_connect(client, &remoteAddress, CTRL_CHANNEL_COUNT, ControlConnectData);
                    if (peer == NULL) {
                        Limelog("PSP: enet_host_connect returned NULL on retry %d\n", enet_retries + 1);
                        break;
                    }
                }

                // Connect to the host
                Limelog("Connecting ENet to %s:%u with data 0x%08x (attempt %d)\n",
                        RemoteAddrString, ControlPortNumber, ControlConnectData, enet_retries + 1);
                ...
                if (err > 0 && event.type == ENET_EVENT_TYPE_CONNECT) {
                    // Success
                    Limelog("ENet connected successfully on attempt %d\n", enet_retries + 1);
                    break;
                }

                // Log the failure
                if (err < 0) {
                    Limelog("ENet attempt %d: error %d\n", enet_retries + 1, LastSocketFail());
                } else if (err == 0) {
                    Limelog("ENet attempt %d: timed out\n", enet_retries + 1);
                } else {
                    Limelog("ENet attempt %d: unexpected event %d (error: %d)\n", enet_retries + 1, (int)event.type, LastSocketError());
                }

                // Only retry on transient disconnect with EAGAIN (PSP-specific)
                if (!(err > 0 && event.type == ENET_EVENT_TYPE_DISCONNECT)) {
                    // Not a retriable event — break immediately
                    break;
                }
            }
...
#endif
        // Hardware stabilization delay (longer on PSP for network stack settle time)
#ifdef _PSP
        PltSleepMs(300);
#else
        PltSleepMs(100);
