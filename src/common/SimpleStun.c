...
int LiFindExternalAddressIP4(const char* stunServer, unsigned short stunPort, unsigned int* wanAddr)
{
...
#else
    he = gethostbyname(stunServer);
    if (he == NULL) {
        Limelog("Failed to resolve STUN server: %s\n", stunServer);
        goto Exit;
    }
    ...
    memcpy(&stunAddr.sin_addr, he->h_addr, he->h_length);
#endif

#ifndef _PSP
    sock = bindUdpSocket(hints.ai_family, NULL, 0, 0, SOCK_QOS_TYPE_BEST_EFFORT);
#else
    sock = bindUdpSocket(AF_INET, NULL, 0, 0, SOCK_QOS_TYPE_BEST_EFFORT);
#endif
...
#else
            err = (int)sendto(sock, (char *)&reqMsg, sizeof(reqMsg), 0, (struct sockaddr*)&stunAddr, sizeof(stunAddr));
            if (err == SOCKET_ERROR) {
                err = LastSocketFail();
                Limelog("Failed to send STUN binding request: %d\n", err);
            }
