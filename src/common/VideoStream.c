...
#define RTP_RECV_PACKETS_BUFFERED 128
...
static void VideoReceiveThreadProc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&receiveThread)) {
        ...
        if (!receivedDataFromPeer) {
            ...
            Limelog("Received first video packet after %d ms (Enc: 0x%X, Supp: 0x%X)\n", 
                    waitingForVideoMs, EncryptionFeaturesEnabled, EncryptionFeaturesSupported);
        ...
        uint32_t typeAndLength = BE32(*(uint32_t*)(encrypted ? encryptedBuffer : buffer));
        if (!encrypted && (typeAndLength & 0x80000000)) {
            static int warned = 0;
            if (!warned) {
                Limelog("WARNING: Received encrypted video packet but SS_ENC_VIDEO is NOT enabled! (typeAndLength: 0x%08X)\n", typeAndLength);
                warned = 1;
            }
        }
...
int startVideoStream(void* rendererContext, int drFlags) {
    ...
    Limelog("AppVersionQuad: %d.%d.%d.%d\n", AppVersionQuad[0], AppVersionQuad[1], AppVersionQuad[2], AppVersionQuad[3]);
    if (AppVersionQuad[0] >= 3) {
        Limelog("Connecting to port %d for first frame...\n", FIRST_FRAME_PORT);
        // Connect this socket to open port 47998 for our ping thread
        firstFrameSocket = connectTcpSocket(&RemoteAddr, AddrLen,
                                            FIRST_FRAME_PORT, FIRST_FRAME_TIMEOUT_SEC);
        if (firstFrameSocket == INVALID_SOCKET) {
#ifdef _PSP
            Limelog("Warning: First frame connection failed on PSP, continuing anyway (Sunshine compatibility)...\n");
#else
            VideoCallbacks.stop();
            stopVideoDepacketizer();
            PltInterruptThread(&receiveThread);
