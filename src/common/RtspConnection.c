...
#define RTSP_CONNECT_TIMEOUT_SEC 20
// PSP1000 / emulation environments can be slow to respond to the initial RTSP OPTIONS.
// Keep this higher so we avoid premature handshake failures.
#define RTSP_RECEIVE_TIMEOUT_SEC 60
#define RTSP_RETRY_DELAY_MS 500
// Retry OPTIONS a few times before aborting the whole handshake.
#define RTSP_OPTIONS_RETRY_COUNT 3
...
static void rtsp_thread_func(void* context);
...
static PLT_THREAD rtsp_thread;
static volatile bool rtsp_thread_running;
...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, bool expectingPayload, int* error) {
    ...
    *error = -1;
    ...
    do {
        ...
        Limelog("RTSP: Connecting to %s:%u (timeout %ds)...\n", urlAddr, RtspPortNumber, RTSP_CONNECT_TIMEOUT_SEC);
        sock = connectTcpSocket(&RemoteAddr, AddrLen, RtspPortNumber, RTSP_CONNECT_TIMEOUT_SEC);
        if (sock == INVALID_SOCKET) {
            Limelog("RTSP: connectTcpSocket failed: %d\n", LastSocketError());
    ...
    Limelog("Sending RTSP message (len %d):\n%.*s\n", messageLen, messageLen > 1000 ? 1000 : messageLen, serializedMessage);
    ...
    // Read the response. For RTSP OPTIONS (no payload) some servers keep the
    // connection open after sending the headers, so waiting for a close can
    // cause timeouts. For expectingPayload==false we attempt to parse early
    // as soon as the RTSP headers are complete.
    offset = 0;
    ...
    for (;;) {
        ...
        if (err < 0) {
            ...
            *error = LastSocketError();
            ...
            if (!expectingPayload && !encryptedRtspEnabled) {
                RTSP_MESSAGE tmp;
                memset(&tmp, 0, sizeof(tmp));
                // If we can fully parse the RTSP response from what we've read
                // so far, stop reading even if the server keeps the socket open.
                if (parseRtspMessage(&tmp, responseBuffer, offset) == RTSP_ERROR_SUCCESS) {
                    freeMessage(&tmp);
                    break;
                }
            }
    ...
    Limelog("RTSP response received (len %d, success %d)\n", offset, ret);
...
Exit:
    ...
    if (ret == false || ConnectionInterrupted) {
        closeSocket(sock);
        sock = INVALID_SOCKET;
    }
#ifdef _PSP
    // PSP: Always close and reconnect for each RTSP transaction.
    // Sunshine closes the TCP connection after each response, and
    // PSP's recv() returns error 113 (ECONNABORTED) if we try to
    // reuse the same socket for the next request (e.g. DESCRIBE
    // after OPTIONS). Forcing a fresh connection avoids this.
    else {
        closeSocket(sock);
        sock = INVALID_SOCKET;
    }
    ...
    *error = -1;
    ...
    if (ret) {
        ...
        if (AppVersionQuad[0] >= 6) {
            ...
            if (strstr(target, "streamid=audio")) {
                clientPort = AudioLocalPort;
            } else if (strstr(target, "streamid=video")) {
                clientPort = VideoLocalPort;
            }
            
            if (clientPort != 0) {
                char transport[128];
                sprintf(transport, "unicast;X-GS-ClientPort=%u-%u", clientPort, clientPort + 1);
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    if (StreamConfig.bitrate >= HIGH_AUDIO_BITRATE_THRESHOLD &&
        ...
        // Use the real remote address for the RTSP target URL.
        // Using 0.0.0.0 can cause some servers to not respond to RTSP OPTIONS.
        // (Audio bitrate negotiation is handled elsewhere; RTSP handshake reliability comes first.)
        addrToUrlSafeString(&RemoteAddr, urlAddr, sizeof(urlAddr));
        snprintf(rtspTargetUrl, sizeof(rtspTargetUrl), "rtsp%s://%s:%u", useEnet ? "ru" : "", urlAddr, RtspPortNumber);
    }
...

    {
        ...
        for (int attempt = 0; attempt < RTSP_OPTIONS_RETRY_COUNT; attempt++) {
            RTSP_MESSAGE response;
            memset(&response, 0, sizeof(response));
            error = -1;

            if (!requestOptions(&response, &error)) {
                Limelog("RTSP OPTIONS attempt %d/%d failed: %d\n",
                    attempt + 1, RTSP_OPTIONS_RETRY_COUNT, error);
                lastFailure = error;
            }
            else if (response.message.response.statusCode != 200) {
                lastFailure = response.message.response.statusCode;
                Limelog("RTSP OPTIONS attempt %d/%d bad status: %d\n",
                    attempt + 1, RTSP_OPTIONS_RETRY_COUNT, lastFailure);
            }
            else {
        ...
        if (!optionsOk) {
            ret = (lastFailure != -1) ? lastFailure : error;
            goto Exit;
        }
    }
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        if (connectData != NULL) {
            ...
            Limelog("ControlConnectData: 0x%08x\n", ControlConnectData);
        }
        else {
            ControlConnectData = 0;
            Limelog("ControlConnectData: NOT FOUND (using 0)\n");
        }
    ...
    // Start external listener thread for Sunshine ANNOUNCE messages
    if (sock != INVALID_SOCKET && !useEnet && ListenerCallbacks.clMessage != NULL) {
        rtsp_thread_running = true;
        PltCreateThread("RtspListener", rtsp_thread_func, NULL, &rtsp_thread);
    }
...
Exit:
    ...
    if (rtsp_thread_running) {
        ...
        // The socket will be closed in terminateRtspConnection, which will wake the thread
    }
...
static void rtsp_thread_func(void* context) {
    (void)context;
    char buffer[2048];
    uint64_t last_keepalive = sceKernelGetSystemTimeWide();
    while (rtsp_thread_running && !ConnectionInterrupted) {
        struct pollfd pfd;
        pfd.fd = sock;
        pfd.events = POLLIN;
        
        int err = pollSockets(&pfd, 1, 1000); // 1s timeout
        
        /* Time-based Keepalive (every 45s) to prevent Sunshine session timeout */
        uint64_t now = sceKernelGetSystemTimeWide();
        if (now - last_keepalive > 45ULL * 1000000ULL) {
            RTSP_MESSAGE keepalive_req, keepalive_res;
            int k_err = -1;
            if (initializeRtspRequest(&keepalive_req, "OPTIONS", "/")) {
                if (sessionIdString != NULL) {
                    addOption(&keepalive_req, "Session", sessionIdString);
                }
                transactRtspMessage(&keepalive_req, &keepalive_res, false, &k_err);
                freeMessage(&keepalive_req);
                freeMessage(&keepalive_res);
            }
            last_keepalive = now;
        }
        ...
        if (err > 0 && (pfd.revents & POLLIN)) {
            ...
            if (received > 0) {
                buffer[received] = '\0';
                /* Look for ANNOUNCE and Sunshine message header */
                if (strstr(buffer, "ANNOUNCE") != NULL) {
                    char* ss_msg = strstr(buffer, "x-ss-message: ");
                    if (ss_msg != NULL) {
                        ss_msg += 14;
                        char* end = strstr(ss_msg, "\r\n");
                        if (end != NULL) {
                            *end = '\0';
                            if (ListenerCallbacks.clMessage != NULL) {
                                ListenerCallbacks.clMessage(ss_msg);
                            }
                            *end = '\r'; // Restore for potential further parsing
                        }
                    }
                }
            } else if (received == 0 || (received < 0 && LastSocketError() != EWOULDBLOCK)) {
...
void terminateRtspConnection(void) {
    rtsp_thread_running = false;
    if (sock != INVALID_SOCKET) {
        closeSocket(sock);
        sock = INVALID_SOCKET;
