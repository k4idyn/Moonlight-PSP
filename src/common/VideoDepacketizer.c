...
static uint64_t waitingForIdrStartTime;
...
void initializeVideoDepacketizer(int pktSize) {
    (void)pktSize;
    LbqInitializeLinkedBlockingQueue(&decodeUnitQueue, 64);
...
static void dropFrameState(void) {
    ...
    if (strictIdrFrameWait || !idrFrameProcessed || waitingForIdrFrame) {
        ...
        if (!waitingForIdrFrame) {
            waitingForIdrFrame = true;
            waitingForIdrStartTime = PltGetMillis();
        }
    }
    else {
    ...
    /* Verbose diag logs disabled for performance
    if (nalChainHead != NULL) {
        char hexBuf[256];
        int offset = 0;
        offset += sprintf(hexBuf + offset, "[DIAG] DROPPING Frame (Head):");
        for (int i = 0; i < 64 && i < (int)nalChainHead->length; i++) {
            offset += sprintf(hexBuf + offset, " %02x", (unsigned char)nalChainHead->data[i]);
        }
        Limelog("%s\n", hexBuf);
    }
    */
...
static void processAvcHevcRtpPayloadSlow(PBUFFER_DESC currentPos, PLENTRY_INTERNAL* existingEntry) {
    ...
    while (currentPos->length != 0) {
        ...
        if (isSeqReferenceFrameStart(currentPos)) {
            Limelog("IDR FRAME FOUND! (type %d)\n", (NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H264) ? 5 : 19);
...
static void processRtpPayload(PNV_VIDEO_PACKET videoPacket, int length,
    ...
    // static uint32_t lastLoggedFrame = 0xFFFFFFFF;
    /* Verbose diag logs disabled for performance
    if (firstPacket && frameIndex != lastLoggedFrame) {
        char hexBuf[256];
        int offset = 0;
        offset += sprintf(hexBuf + offset, "[DIAG] Frame %u Head (raw):", frameIndex);
        for (int i = 0; i < 64 && i < (int)length; i++) {
            offset += sprintf(hexBuf + offset, " %02x", (unsigned char)((char*)videoPacket)[i]);
        }
        Limelog("%s\n", hexBuf);
    ...
    */
    ...
    if (!formatLogged) {
        Limelog("[DIAG] NegotiatedVideoFormat: 0x%08X\n", NegotiatedVideoFormat);
        formatLogged = true;
    }
    */
    ...
    if (firstPacket && currentPos.length > 0) {
        ...
        char hexBuf[256];
        int offset = 0;
        offset += sprintf(hexBuf + offset, "[DIAG] Frame %u Head (post-NV):", frameIndex);
        for (int i = 0; i < 64 && i < (int)currentPos.length; i++) {
            offset += sprintf(hexBuf + offset, " %02x", (unsigned char)currentPos.data[i]);
        }
        Limelog("%s\n", hexBuf);
        */
    ...
    if (lastPacket) {
        ...
        if (waitingForIdrFrame || waitingForRefInvalFrame) {
            ...
            if (waitingForIdrFrame) {
                if (PltGetMillis() - waitingForIdrStartTime > 5000) {
                    Limelog("IDR frame request timeout. Re-requesting... (Format: 0x%X)\n", NegotiatedVideoFormat);
                    LiRequestIdrFrame();
                    waitingForIdrStartTime = PltGetMillis();
                }
                ...
                if (getAnnexBStartSequence(&currentPos, &startSeq)) {
                    int type = (NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H264) ? 
                                H264_NAL_TYPE(startSeq.data[startSeq.offset + startSeq.length]) :
                                HEVC_NAL_TYPE(startSeq.data[startSeq.offset + startSeq.length]);
                    if (frameIndex % 60 == 0 && lastPacket) {
                        Limelog("Waiting for IDR frame. Last NAL type in frame %d: %d\n", frameIndex, type);
                    }
                } else {
                    if (frameIndex % 60 == 0 && lastPacket && currentPos.length >= 8) {
                        Limelog("Waiting for IDR frame. No Annex B found! Head: %02x %02x %02x %02x %02x %02x %02x %02x\n",
                                (unsigned char)currentPos.data[0], (unsigned char)currentPos.data[1],
                                (unsigned char)currentPos.data[2], (unsigned char)currentPos.data[3],
                                (unsigned char)currentPos.data[4], (unsigned char)currentPos.data[5],
                                (unsigned char)currentPos.data[6], (unsigned char)currentPos.data[7]);
                    }
                }
