...
#elif defined(_PSP)
int ThreadProcPSP(SceSize args, void *argp) {
    (void)args;
...
#if defined(LC_WINDOWS) || defined(__vita__) || defined(__WIIU__) || defined(__3DS__) || defined(_PSP)
    return 0;
#else
    return NULL;
#endif
...
#elif defined(_PSP)
    *mutex = sceKernelCreateSema("PltMutex", 0, 1, 1, NULL);
    if (*mutex < 0) {
        return *mutex;
    }
#else
...
#elif defined(_PSP)
    sceKernelWaitThreadEnd(thread->thread, NULL);
    sceKernelDeleteThread(thread->thread);
#else
...
int PltCreateThread(const char* name, ThreadEntry entry, void* context, PLT_THREAD* thread) {
...
#elif defined(_PSP)
    {
        ...
        sceKernelStartThread(thread->thread, sizeof(ctx), &ctx);
    }
#else
    {
        pthread_attr_t attr;
...
int PltCreateConditionVariable(PLT_COND* cond, PLT_MUTEX* mutex) {
    ...
    CondVar_Init(cond);
#elif defined(_PSP)
    *cond = sceKernelCreateEventFlag("PltCond", 0, 0, NULL);
    if (*cond < 0) {
        return *cond;
    }
#else
...
#elif defined(_PSP)
    // Simple CV simulation with EventFlags. 
    // This is not perfect (no atomic unlock+wait), but should work for Moonlight's usage.
    PltUnlockMutex(mutex);
    sceKernelWaitEventFlag(*cond, 1, PSP_EVENT_WAITAND | PSP_EVENT_WAITCLEAR, NULL, NULL);
    PltLockMutex(mutex);
#else
