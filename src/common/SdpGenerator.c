...
static PSDP_OPTION getAttributesList(char*urlSafeAddr) {
    ...
    if (IS_SUNSHINE()) {
        ...
        // PSP modification: Respect client encryption flags to allow disabling AES-GCM on weaker hardware
        if ((EncryptionFeaturesSupported & SS_ENC_CONTROL_V2) && StreamConfig.encryptionFlags != 0) {
...
#ifdef _PSP
    uint16_t port = (VideoLocalPort != 0) ? VideoLocalPort : VideoPortNumber;
    return snprintf(buffer, length,
        "t=0 0\r\n"
        "m=video %d  \r\n",
        AppVersionQuad[0] < 4 ? 47996 : port);
#else
