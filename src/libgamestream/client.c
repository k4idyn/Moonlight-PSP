...
static int load_device_name(const char* keyDirectory) {
    strncpy(device_name, "roth", sizeof(device_name) - 1);
    device_name[sizeof(device_name) - 1] = '\0';
    
    char path[PATH_MAX];
    snprintf(path, sizeof(path), "%s/devicename.dat", keyDirectory);
    
    SceUID fd = sceIoOpen(path, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
    if (fd >= 0) {
        sceIoWrite(fd, device_name, strlen(device_name));
        sceIoClose(fd);
        sceIoSync("ms0:", 0);
    }
    return GS_OK;
...
static int mkdirtree(const char* directory) {
        ...
        *p = 0;
        ...
        if (!should_skip && buffer[0] != '\0') {
            ...
            if (sceIoGetstat(buffer, &stat) < 0) {
                ...
                if (res == 0) {
                    LOG_INFO(COMPONENT_NETWORK, "mkdirtree: Created directory %s", buffer);
                } else if (res != (int)0x80010011) { // 0x80010011 is EEXIST
                    LOG_ERROR(COMPONENT_NETWORK, "mkdirtree: Failed to create %s (0x%08X)", buffer, res);
                }
            }
        }
        *p = oldChar;
  ...
  if (sceIoGetstat(buffer, &stat) < 0) {
      int res = sceIoMkdir(buffer, 0777);
      if (res == 0) {
          LOG_INFO(COMPONENT_NETWORK, "mkdirtree: Created final directory %s", buffer);
      } else if (res != (int)0x80010011) {
          LOG_ERROR(COMPONENT_NETWORK, "mkdirtree: Failed to create final %s (0x%08X)", buffer, res);
      }
  }
...
static int load_unique_id(const char* keyDirectory) {
  ...
  char uuidFilePath[PATH_MAX];
  snprintf(uniqueFilePath, PATH_MAX, "%s/%s", keyDirectory, UNIQUE_FILE_NAME);
  snprintf(uuidFilePath, PATH_MAX, "%s/%s", keyDirectory, UUID_FILE_NAME);

  /* Load or generate Unique ID */
  SceUID fd = sceIoOpen(uniqueFilePath, PSP_O_RDONLY, 0);
  if (fd >= 0) {
    int read_len = sceIoRead(fd, unique_id, UNIQUEID_CHARS);
    sceIoClose(fd);
    if (read_len == UNIQUEID_CHARS) {
        unique_id[UNIQUEID_CHARS] = 0;
        LOG_INFO(COMPONENT_NETWORK, "Unique ID loaded from %s", uniqueFilePath);
    } else {
        LOG_ERROR(COMPONENT_NETWORK, "Partial read of %s (got %d bytes), regenerating.", uniqueFilePath, read_len);
        fd = -1; // Trigger regeneration
    }
  } else {
      LOG_INFO(COMPONENT_NETWORK, "uniqueid.dat not found at %s (error 0x%08x), generating new one.", uniqueFilePath, (int)fd);
  }

  if (fd < 0) {
    unsigned char random_bytes[UNIQUEID_BYTES];
    RAND_bytes(random_bytes, UNIQUEID_BYTES);
    snprintf(unique_id, UNIQUEID_CHARS + 1, "0123456789ABCDEF");

    fd = sceIoOpen(uniqueFilePath, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
    if (fd >= 0) {
      sceIoWrite(fd, unique_id, UNIQUEID_CHARS);
      sceIoClose(fd);
      sceIoSync("ms0:", 0);
      LOG_INFO(COMPONENT_NETWORK, "New Unique ID saved to %s", uniqueFilePath);
    } else {
      LOG_ERROR(COMPONENT_NETWORK, "Failed to save uniqueid.dat (error 0x%08x) at %s", (int)fd, uniqueFilePath);
    }
  }

  /* Load or generate UUID */
  fd = sceIoOpen(uuidFilePath, PSP_O_RDONLY, 0);
  if (fd >= 0) {
    int read_len = sceIoRead(fd, uuid_str, UUID_STRLEN-1);
    sceIoClose(fd);
    if (read_len == (UUID_STRLEN-1)) {
        uuid_str[UUID_STRLEN-1] = '\0';
    } else {
        LOG_ERROR(COMPONENT_NETWORK, "Partial read of %s (got %d bytes), regenerating.", uuidFilePath, read_len);
        fd = -1;
    }
  }
  ...
  if (fd < 0) {
    ...
    RAND_bytes(uuid_data, sizeof(uuid_data));
    snprintf(uuid_str, UUID_STRLEN, 
    ...
    fd = sceIoOpen(uuidFilePath, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
    if (fd >= 0) {
      sceIoWrite(fd, uuid_str, UUID_STRLEN-1);
      sceIoClose(fd);
      sceIoSync("ms0:", 0);
    }
  }

  LOG_INFO(COMPONENT_NETWORK, "UNIQUE ID: %s", unique_id);
  LOG_INFO(COMPONENT_NETWORK, "UUID STR: %s", uuid_str);
  return GS_OK;
}
...
static int load_cert(const char* keyDirectory) {
  ...
  snprintf(certPath, sizeof(certPath), "%s/%s", keyDirectory, CERTIFICATE_FILE_NAME);
  ...
  SceUID fd = sceIoOpen(derFile, PSP_O_RDONLY, 0);
  if (fd >= 0) {
      der_len = sceIoRead(fd, pem_buf, sizeof(pem_buf));
      sceIoClose(fd);
      if (der_len > 0) {
          der_loaded = true;
          LOG_INFO(COMPONENT_NETWORK, "Loaded DER certificate from %s (%d bytes)", derFile, der_len);
      }
  }
  ...
  if (!der_loaded) {
      ...
      if (fd >= 0) {
          int pem_len = sceIoRead(fd, pem_buf, sizeof(pem_buf) - 1);
          sceIoClose(fd);
          if (pem_len > 0) {
              pem_buf[pem_len] = '\0';
              LOG_INFO(COMPONENT_NETWORK, "Loaded PEM certificate from %s, converting to DER...", certPath);
              
              mbedtls_x509_crt temp_crt;
              mbedtls_x509_crt_init(&temp_crt);
              int res = mbedtls_x509_crt_parse(&temp_crt, (unsigned char*)pem_buf, pem_len + 1);
              if (res == 0) {
                  // Re-export as DER to pem_buf for cert_hex
                  der_len = temp_crt.raw.len;
                  if (der_len <= (int)sizeof(pem_buf)) {
                      memcpy(pem_buf, temp_crt.raw.p, der_len);
                      der_loaded = true;
                      
                      // Save it as .der for next time
                      fd = sceIoOpen(derFile, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
                      if (fd >= 0) {
                          sceIoWrite(fd, pem_buf, der_len);
                          sceIoClose(fd);
                          sceIoSync("ms0:", 0);
                          LOG_INFO(COMPONENT_NETWORK, "Saved DER certificate for future use.");
                      }
                  }
              }
              mbedtls_x509_crt_free(&temp_crt);
          }
      }
  }

  if (!der_loaded) {
    LOG_INFO(COMPONENT_NETWORK, "No valid certificate found. Generating new identity in %s...", keyDirectory);
    CERT_KEY_PAIR cert_pair = mkcert_generate(unique_id);
    LOG_INFO(COMPONENT_NETWORK, "Certificate generation complete.");

    mkcert_save(certPath, p12Path, keyPath, cert_pair);
    
    // Load the brand new DER
  ...
  // Convert DER binary in pem_buf to HEX (LOWERCASE for Sunshine cert string)
  int length = 0;
  for (int i = 0; i < der_len && length < (int)sizeof(cert_hex) - 2; i++) {
    snprintf(cert_hex + length, 3, "%02x", (unsigned char)pem_buf[i]);
    length += 2;
  }
  cert_hex[length] = '\0';

  // Also parse it into the global 'cert' for TLS requests
  if (cert == NULL) {
  ...
  int crt_parse_res = mbedtls_x509_crt_parse(cert, (unsigned char*)pem_buf, der_len);
  if (crt_parse_res != 0) {
      char err_buf[128];
      mbedtls_strerror(crt_parse_res, err_buf, sizeof(err_buf));
      LOG_ERROR(COMPONENT_NETWORK, "mbedtls_x509_crt_parse FAILED: -0x%04x (%s)", -crt_parse_res, err_buf);
  ...
  if (key_fd >= 0) {
      ...
      if (key_read > 0) {
          ...
          // Note: mbedtls_pk_parse_key needs the null terminator for PEM, which we have from memset
          int pk_res = mbedtls_pk_parse_key(privateKey, (unsigned char*)key_pem, key_read + 1, NULL, 0, mbedtls_ctr_drbg_random, NULL);
          if (pk_res != 0) {
              char err_buf[128];
              mbedtls_strerror(pk_res, err_buf, sizeof(err_buf));
              LOG_ERROR(COMPONENT_NETWORK, "mbedtls_pk_parse_key FAILED: -0x%04x (%s)", -pk_res, err_buf);
  ...
  // mbedtls_x509_crt_parse removed here as it was moved above and deduplicated
...
static int load_serverinfo(PSERVER_DATA server, bool https) {
  ...
  if (http_request(url, data) != GS_OK) {
    if ((flog=fopen("moonlight_debug.log", "a"))) { fprintf(flog, "[CLIENT] http_request failed for: %s\n", url); fclose(flog); }
    ret = GS_IO_ERROR;
  ...
  if (flog) { fprintf(flog, "XML payload received:\n%s\n", data->memory); fclose(flog); }

  if (xml_status(data->memory, data->size) == GS_ERROR) {
    if ((flog=fopen("moonlight_debug.log", "a"))) { fprintf(flog, "[CLIENT] xml_status check failed for: %s\n", url); fclose(flog); }
    ret = GS_ERROR;
  ...
  if (flog2) {
      fprintf(flog2, "currentGameText: '%s', pairedText: '%s', appVersion: '%s', stateText: '%s'\n",
              currentGameText, pairedText, server->serverInfo.serverInfoAppVersion, stateText);
      fclose(flog2);
  }
  ...
  if (strstr(stateText, "MJOLNIR") == NULL) {
    // Sunshine detection - force a fake version that triggers modern features and IS_SUNSHINE().
    // We use -1 for the last quad to trigger IS_SUNSHINE() in moonlight-common-c.
    LOG_INFO(COMPONENT_NETWORK, "Sunshine detected! Forcing version 7.1.431.-1 for compatibility.");
    free((void*)server->serverInfo.serverInfoAppVersion);
    server->serverInfo.serverInfoAppVersion = strdup("7.1.431.-1");
    // Standard Forensic Repair: Explicitly set the Sunshine flag for IS_SUNSHINE() macro
    AppVersionQuad[3] = -1;
  }
  ...
  /* Note: Sunshine doesn't always signal MJOLNIR_SERVER_BUSY when an app is open 
   * but not streaming. We preserve server->currentGame if it's non-zero in XML. */
...
static int load_server_status(PSERVER_DATA server) {
  ...
  for (i = 0; i < 2 && ret != GS_OK; i++) {
    ...
    LOG_INFO(COMPONENT_NETWORK, "Attempting serverinfo via %s...", use_https ? "HTTPS" : "HTTP");
    ret = load_serverinfo(server, use_https);
    if (ret != GS_OK) {
        LOG_INFO(COMPONENT_NETWORK, "  %s attempt failed (err=%d)", use_https ? "HTTPS" : "HTTP", ret);
    } else {
        LOG_INFO(COMPONENT_NETWORK, "  %s attempt SUCCESS!", use_https ? "HTTPS" : "HTTP");
    }
  }
...
static void hex_to_bytes(const char *in, unsigned char* out, size_t len) {
  ...
  for (count = 0; count < len; count += 2) {
    out[count / 2] = (hex_char_to_val(in[count]) << 4) | hex_char_to_val(in[count+1]);
  }
}

static int sign_it(const unsigned char *msg, size_t mlen, unsigned char **sig, size_t *slen, mbedtls_pk_context *pkey) {
  unsigned char hash[32];
  mbedtls_sha256(msg, mlen, hash, 0);

  *sig = malloc(MBEDTLS_PK_SIGNATURE_MAX_SIZE);
  if (*sig == NULL) return GS_FAILED;
  ...
  if (mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_rand, &entropy, (const unsigned char *) "PSPMoonlightSign", 16) != 0) {
      free(*sig);
      *sig = NULL;
  ...
  if (mbedtls_pk_sign(pkey, MBEDTLS_MD_SHA256, hash, 32, *sig, MBEDTLS_PK_SIGNATURE_MAX_SIZE, slen, mbedtls_ctr_drbg_random, &ctr_drbg) != 0) {
      free(*sig);
      *sig = NULL;
      mbedtls_ctr_drbg_free(&ctr_drbg);
      mbedtls_entropy_free(&entropy);
...
static bool verifySignature(const unsigned char *data, int dataLength, const unsigned char *signature, int signatureLength, const char *cert, size_t certLen) {
    mbedtls_x509_crt x509;
    mbedtls_x509_crt_init(&x509);
    
    int parse_ret = mbedtls_x509_crt_parse(&x509, (const unsigned char*)cert, certLen);
    if (parse_ret != 0) {
        // Try parsing as PEM by including the null terminator in the length, as required by mbedtls
        parse_ret = mbedtls_x509_crt_parse(&x509, (const unsigned char*)cert, certLen + 1);
    }
    
    if (parse_ret != 0) {
        FILE *fdbg = fopen("moonlight_debug.log", "a");
        if (fdbg) { fprintf(fdbg, "verifySignature: crt_parse failed: -0x%x\n", -parse_ret); fclose(fdbg); }
    ...
    int result = mbedtls_pk_verify(&x509.pk, MBEDTLS_MD_SHA256, hash, 32, signature, signatureLength);
    if (result != 0) {
        FILE *fdbg = fopen("moonlight_debug.log", "a");
        if (fdbg) { fprintf(fdbg, "verifySignature: pk_verify failed: -0x%x\n", -result); fclose(fdbg); }
    }
...
static void encrypt(const unsigned char *plaintext, int plaintextLen, const unsigned char *key, unsigned char *ciphertext) {
    ...
    for (i = 0; i < plaintextLen; i += 16) {
        mbedtls_aes_crypt_ecb(&ctx, MBEDTLS_AES_ENCRYPT, plaintext + i, ciphertext + i);
    }
...
int gs_unpair(PSERVER_DATA server) {
  ...
  snprintf(url, sizeof(url), "%s://%s:%u/unpair?uniqueid=%s&uuid=%s", use_https ? "https" : "http", server->serverInfo.address, use_https ? server->httpsPort : server->httpPort, unique_id, uuid_str);
...
int gs_pair(PSERVER_DATA server, char* pin) {
  ...
  snprintf(url, url_max_len, "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&updateState=1&phrase=getservercert&salt=%s&clientcert=%s", 
           server->serverInfo.address, server->httpPort, unique_id, uuid_str, "roth", "roth", salt_hex, cert_hex);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 1 - Sending getservercert (salt + clientcert)...");
  data = http_create_data();
  if (data == NULL)
    return GS_OUT_OF_MEMORY;
  else if ((ret = http_request(url, data)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: Step 1 http_request FAILED ret=%d", ret);
    goto cleanup;
  }
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 1 response received (%d bytes)", data->size);
  ...
  snprintf(url, url_max_len, "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&updateState=1&clientchallenge=%s", server->serverInfo.address, server->httpPort, unique_id, uuid_str, "roth", "roth", challenge_hex);
  ...
  if (data->size > 0 && data->size < 1024) {
      LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 2 XML: %s", (char*)data->memory);
  }
  ...
  free(result);
  result = NULL;
  if ((ret = xml_search(data->memory, data->size, "challengeresponse", &result)) != GS_OK) {
    ret = GS_INVALID;
    goto cleanup;
  }
  ...
  hex_to_bytes(result, (unsigned char*)challenge_response_data_enc, strlen(result));
  decrypt((unsigned char*)challenge_response_data_enc, sizeof(challenge_response_data_enc), aes_key, (unsigned char*)challenge_response_data);
  ...
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: cert->sig.len = %d", (int)cert->sig.len);
  bytes_to_hex(cert->sig.p, sig_start_hex, 16);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: cert->sig.p[0..15] = %s", sig_start_hex);
  
  memcpy(challenge_response, challenge_response_data + hash_length, 16);
  memcpy(challenge_response + 16, cert->sig.p, cert->sig.len);
  memcpy(challenge_response + 16 + cert->sig.len, (unsigned char*)client_secret_data, sizeof(client_secret_data));
  if (server->serverMajorVersion >= 7)
    mbedtls_sha256((unsigned char*)challenge_response, 16 + cert->sig.len + sizeof(client_secret_data), (unsigned char*)challenge_response_hash, 0);
  else
    mbedtls_sha1((unsigned char*)challenge_response, 16 + cert->sig.len + sizeof(client_secret_data), (unsigned char*)challenge_response_hash);

  encrypt((unsigned char*)challenge_response_hash, sizeof(challenge_response_hash), aes_key, (unsigned char*)challenge_response_hash_enc);
  bytes_to_hex((unsigned char*)challenge_response_hash_enc, challenge_response_hex, sizeof(challenge_response_hash_enc));

  snprintf(url, url_max_len, "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&updateState=1&serverchallengeresp=%s", server->serverInfo.address, server->httpPort, unique_id, uuid_str, "roth", "roth", challenge_response_hex);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 3 - Sending serverchallengeresp...");
  if ((ret = http_request(url, data)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: Step 3 http_request FAILED ret=%d", ret);
    goto cleanup;
  }
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 3 response received (%d bytes)", data->size);
  ...
  if (!verifySignature((unsigned char*)pairing_secret, 16, (unsigned char*)pairing_secret+16, pairing_secret_len-16, plaincert, plaincertlen)) {
  ...
  size_t s_len;
  if (sign_it((unsigned char*)client_secret_data, sizeof(client_secret_data), &signature, &s_len, privateKey) != GS_OK) {
  ...
  memcpy(client_pairing_secret, (unsigned char*)client_secret_data, sizeof(client_secret_data));
  ...
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 - client_secret_data: %02X%02X%02X%02X...", 
           (unsigned char)client_secret_data[0], (unsigned char)client_secret_data[1], 
           (unsigned char)client_secret_data[2], (unsigned char)client_secret_data[3]);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 - Signature length: %u, head: %02X%02X%02X%02X", 
           (unsigned int)s_len, signature[0], signature[1], signature[2], signature[3]);

  snprintf(url, url_max_len, "http://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&updateState=1&clientpairingsecret=%s", server->serverInfo.address, server->httpPort, unique_id, uuid_str, "roth", "roth", client_pairing_secret_hex);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 - Sending clientpairingsecret...");
  if ((ret = http_request(url, data)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: Step 4 http_request FAILED ret=%d", ret);
    goto cleanup;
  }
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 response received (%d bytes)", data->size);
  if (data->size > 0) {
      ...
      LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 Response Payload: %s", log_snippet);
  }
  ...
  if (data->size > 0 && data->size < 4096) {
      LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 XML: %s", (char*)data->memory);
  }
  if ((ret = xml_status(data->memory, data->size)) != GS_OK) {
    if (status_msg && strlen(status_msg) > 0) {
        LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 XML status failed (%d), Sunshine Error: %s", ret, status_msg);
        gs_error = status_msg;
        ...
        LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 XML status failed (%d), checking for Sunshine manual approval", ret);
        if (IS_SUNSHINE()) {
            for (int i = 120; i > 0; i--) {
                snprintf(countdown_msg, sizeof(countdown_msg), "Sunshine needs approval. Waiting... (%ds)", i);
                ...
                if (http_request(url, &tmp_data) == GS_OK) {
                    if (tmp_data.size > 0) {
                        LOG_INFO(COMPONENT_NETWORK, "Manual Poll Response: %s", (char*)tmp_data.memory);
                    }
                    char *tmp_result = NULL;
                    if (xml_search(tmp_data.memory, tmp_data.size, "paired", &tmp_result) == GS_OK && tmp_result && !strcmp(tmp_result, "1")) {
                        LOG_INFO(COMPONENT_NETWORK, "Manual approval detected! Pairing SUCCESS.");
                        ret = GS_OK;
                ...
                if (tmp_data.memory) free(tmp_data.memory);
    ...
    LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 paired tag not found, but ignoring for Sunshine");
    ret = GS_OK;
  }

step5: ; // Label now outside the block or with dummy statement
  if (result == NULL || strcmp(result, "1") != 0) {
    LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 4 returned paired=%s", result ? result : "NULL");
    if (!IS_SUNSHINE()) {
        ret = GS_FAILED;
    }
  }

  snprintf(url, url_max_len, "https://%s:%u/pair?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&updateState=1&phrase=pairchallenge", 
           server->serverInfo.address, server->httpsPort, unique_id, uuid_str, "roth", "roth");
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Final HTTPS pairchallenge URL: https://%s:%u/pair?...phrase=pairchallenge", server->serverInfo.address, server->httpsPort);
  LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 5 - Sending HTTPS pairchallenge (confirmation)...");
  if ((ret = http_request(url, data)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: HTTPS pairchallenge FAILED (ret=%d). (non-fatal for Sunshine)", ret);
  ...
  result = NULL;
  if ((ret = xml_status(data->memory, data->size)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: Step 5 XML status failed (non-fatal)");
    ret = GS_OK;
    server->paired = true;
    goto cleanup;
  } else if ((ret = xml_search(data->memory, data->size, "paired", &result)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_pair: Step 5 paired tag not found (non-fatal)");
    ret = GS_OK;
    server->paired = true;
    goto cleanup;
  }
  ...
  if (result == NULL || strcmp(result, "1") != 0) {
    LOG_INFO(COMPONENT_NETWORK, "gs_pair: Step 5 returned paired=%s (non-fatal on Sunshine)", result ? result : "NULL");
    ret = GS_OK;
  }
...
int gs_applist(PSERVER_DATA server, PAPP_LIST *list) {
  ...
  snprintf(url, sizeof(url), "https://%s:%u/applist?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s", server->serverInfo.address, server->httpsPort, unique_id, uuid_str, "roth", "roth");
  if (http_request(url, data) != GS_OK)
    ret = GS_IO_ERROR;
  else {
    LOG_INFO(COMPONENT_NETWORK, "gs_applist response: %.256s", (char*)data->memory);
    if (xml_status(data->memory, data->size) == GS_ERROR) {
      LOG_INFO(COMPONENT_NETWORK, "gs_applist: XML status check failed (non-fatal for Sunshine)");
...
int gs_start_app(PSERVER_DATA server, STREAM_CONFIGURATION *config, int appId, bool sops, bool localaudio, int gamepad_mask) {
  ...
  while (mode != NULL) {
    if (mode->width == (unsigned int)config->width && mode->height == (unsigned int)config->height) {
  ...
  if (!correct_mode && !server->unsupported) {
    ...
    // However, we want to allow the PSP to request arbitrary resolutions (like 360x204)
    // to utilize Sunshine's "Virtual Display" feature. So we log it and proceed anyway.
    correct_mode = true;
  }
  ...
  /* Use the resolution requested by the caller (PSP defaults to 480x272/360x204).
   * Hardcoding 1280x720 here was a legacy hack that causes PSP 1000 to crash 
   * due to bitstream overhead and RTSP timeouts. */
  int request_w = config->width;
  int request_h = config->height;
  
  snprintf(url, sizeof(url), "https://%s:%u/%s?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s&appid=%d&mode=%dx%dx%d&additionalStates=1&sops=%d&rikey=%s&rikeyid=%d&localAudioPlayMode=%d&surroundAudioInfo=%d&remoteControllersBitmap=%d&gcmap=%d%s",
           server->serverInfo.address, server->httpsPort, server->currentGame ? "resume" : "launch", unique_id, uuid_str, "roth", "roth", appId, request_w, request_h, fps, sops, rikey_hex, rikeyid, localaudio, surround_info, gamepad_mask, gamepad_mask,
  ...
  LOG_INFO(COMPONENT_NETWORK, "gs_start_app response: %s", (char*)data->memory);
  if ((ret = xml_status(data->memory, data->size)) != GS_OK) {
    LOG_INFO(COMPONENT_NETWORK, "gs_start_app: XML status check failed (%d).", ret);
    /* We don't force GS_OK here anymore to prevent dummy RTSP connections.
       We will still try to find sessionUrl0 below, and if found, we set ret = GS_OK. */
  }
  
  if ((ret = xml_search(data->memory, data->size, "gamesession", &result)) != GS_OK &&
           (ret = xml_search(data->memory, data->size, "resume", &result)) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_start_app: Failed to find gamesession/resume tag");
    goto cleanup;
  }


  if (result == NULL || !strcmp(result, "0")) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_start_app: gamesession/resume check failed (result=%s)", result ? result : "NULL");
    ret = GS_FAILED;
  ...
  if (xml_search(data->memory, data->size, "sessionUrl0", &result) == GS_OK && result != NULL) {
    if (server->serverInfo.rtspSessionUrl) {
        free((char*)server->serverInfo.rtspSessionUrl);
    }
    server->serverInfo.rtspSessionUrl = result;
    LOG_INFO(COMPONENT_NETWORK, "gs_start_app: RTSP Session URL identified: %s", (char*)server->serverInfo.rtspSessionUrl);
    result = NULL;
  } else {
    /* If we have no session URL, we can't connect, so this is a failure even if resume was 1 */
    LOG_ERROR(COMPONENT_NETWORK, "gs_start_app: Missing sessionUrl0 in response");
    ret = GS_FAILED;
  }
...
int gs_quit_app(PSERVER_DATA server) {
  ...
  snprintf(url, sizeof(url), "https://%s:%u/cancel?uniqueid=%s&uuid=%s&devicename=%s&clientname=%s", server->serverInfo.address, server->httpsPort, unique_id, uuid_str, "PSPMoonlight", "PSPMoonlight");
  if ((ret = http_request(url, data)) != GS_OK)
    goto cleanup;
...
int gs_init(PSERVER_DATA server, char *address, unsigned short httpPort, const char *keyDirectory, int log_level, bool unsupported) {
  ...
  if (load_unique_id(keyDirectory) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_init: load_unique_id FAILED");
    return GS_FAILED;
  }

  if (load_device_name(keyDirectory) != GS_OK) {
    LOG_ERROR(COMPONENT_NETWORK, "gs_init: load_device_name FAILED");
  }
  ...
  LOG_INFO(COMPONENT_NETWORK, "gs_init: server status=%d (paired=%d)", ret, server->paired);
  return ret;
}
...
void gs_cleanup(PSERVER_DATA server) {
  ...
  if (server->gpuType != NULL) { free(server->gpuType); server->gpuType = NULL; }
  if (server->gsVersion != NULL) { free(server->gsVersion); server->gsVersion = NULL; }
